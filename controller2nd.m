...
Weta = [(10:5:40), (80:5:200)];    % frequenze insieme eta
M_min = 0.29;                       % modulus margin
alpha_value = 0.03;
...
% ----------------------------- 2ND ORDER ---------------------------------
n_k1 = 3; % numero di elementi di k1
n_k2 = 80; % numero di elementi di k2
...
C_optm=optimized_k1*(1 + optimized_k2*s)^2/(1 + optimized_k2*optimized_k3*s)^2;
L_optm=C_optm*P;
...
s = tf('s');
C = k1*(1 + k2*s)^2/(1 + alpha_value*k2*s)^2;
L = C*P; 
end